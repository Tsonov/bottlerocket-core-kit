...
async fn generate_cluster_dns_ip(
    ...
    println!("generate_cluster_dns_ip::retrieving EKS network configuration");
...
async fn get_eks_network_config(aws_k8s_info: &SettingsViewDelta) -> Result<Option<String>> {
    println!("get_eks_network_config::start");
    if let (Some(region), Some(cluster_name)) = (
        ...
        println!("get_eks_network_config::getting cluster network config");
        ...
        {
            println!("get_eks_network_config::eks error, fallback to extracting from CIDR");
    ...
    println!("get_eks_network_config::end with no value");
    Ok(None)
}
...
async fn run() -> Result<()> {
    ...
    println!("Starting pluto");

    println!("Creating IMDS client");
    let mut client = ImdsClient::new();
    println!("Getting EKS metadata from bottlerocket api");
    ...
    //println!("aws_k8s_info {:?}", aws_k8s_info);



    println!("Installing AWS_LC cryptographic provider");
...
async fn main() {
    if let Err(e) = run().await {
        println!("Pluto main encountered error");
        eprintln!("{}", e);
        process::exit(1);
    }
    println!("Pluto main finished");
}
